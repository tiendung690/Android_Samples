...
package jp.ohwada.android.vision5.util;
...
import jp.ohwada.android.vision5.ui.AutoFitTextureView;
 ...
 */
  ...
  */
    protected void setUpCameraOutputs(int width, int height) {
        ...
        prev_log(  "LargestSize: " +mLargestSize.toString() );
 ...
 * chooseLargestSize
 */
protected Size chooseLargestSize( StreamConfigurationMap  map ) {
        prev_log("chooseLargestSize");
    // For still image captures, we use the largest available size.
...
protected void openCamera() {
    ...
    // check permission
    if (!CameraPerm.isCameraGranted(mContext)) {
                prev_log("not granted camera permission");
                return;
    }
    if (mCameraId == null) {
