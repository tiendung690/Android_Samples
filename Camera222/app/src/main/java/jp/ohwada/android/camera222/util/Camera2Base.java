 ...
 * 2019-08-01 K.OHWADA
 */
package jp.ohwada.android.camera222.util;
...
import jp.ohwada.android.camera222.ui.AutoFitTextureView;
...
public class Camera2Base {
 ...
 /**
    ...
    public static final int CAMERA_AE_OFF = CaptureRequest.CONTROL_AE_MODE_OFF;
    public static final int CAMERA_AE_ON = CaptureRequest.CONTROL_AE_MODE_ON;
    public static final int CAMERA_AE_AUTO_FLASH = CaptureRequest.CONTROL_AE_MODE_ON_AUTO_FLASH;
    public static final int CAMERA_AE_ON_ALWAYS_FLASH = CaptureRequest.CONTROL_AE_MODE_ON_ALWAYS_FLASH;
    public static final int CAMERA_AE_AUTO_FLASH_REDEYE = CaptureRequest.CONTROL_AE_MODE_ON_AUTO_FLASH_REDEYE;
    protected int mFlashMode = CAMERA_AE_AUTO_FLASH;
    ...
    // AWB : Auto white Baance mode
//Value: 0:
public final static int CAMERA_AWB_OFF = CaptureRequest.CONTROL_AWB_MODE_OFF;
//  Value: 1:
public final static int CAMERA_AWB_AUTO = CaptureRequest.CONTROL_AWB_MODE_AUTO;
 ...
 */
  ...
  */
    protected void setUpCameraOutputs(int width, int height) {
        ...
        log_base("SensorOrientation: " + mSensorOrientation);
...
protected int getAutoFocusPictureMode() {
        ...
        int mode = CAMERA_AF_OFF;
        if(isAutoFocusContinuousPictureSupported) {
            mode = CAMERA_AF_CONTINUOUS_PICTURE ;
        } else if(isAutoFocusSupported) {
            mode = CAMERA_AF_AUTO;
        }
        log_base("getAutoFocusPictureMode: " + mode);
        return mode;
}
...
protected int getAutoExposureFlashMode() {
        ...
        int mode = CAMERA_AE_OFF;
        if (isAutoFlashSupported) {
            mode = CAMERA_AE_AUTO_FLASH;
        } else if (isAutoExposureOnSupported) {
            mode = CAMERA_AE_ON;
        }
        log_base("getAutoExposureFlashMode: " + mode);
        return mode;
}
...
protected int getAutoWhiteBalanceMode() {
        ...
        int mode = CAMERA_AWB_OFF;
        if (isAutoWhiteBalnceSupported) {
            mode = CAMERA_AWB_AUTO;
        }
        log_base("getAutoWhiteBalanceMode: " + mode);
        return mode;
}
